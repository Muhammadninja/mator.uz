...
describe('isAllowedAssetUrl', () => {
  ...
  it('accepts a subdomain of an allowlisted host', () => {
    expect(
      isAllowedAssetUrl('https://res.cloudinary.com/demo/image/x.jpg', allow),
    ).toBe(true);
  });

  it('is case-insensitive on the host', () => {
    expect(isAllowedAssetUrl('https://RES.Cloudinary.COM/x.jpg', allow)).toBe(
      true,
    );
  });
  ...
  it('rejects hosts not on the allowlist', () => {
    expect(isAllowedAssetUrl('https://evil.example.com/a.png', allow)).toBe(
      false,
    );
  });
  ...
  it('rejects a look-alike host that only ends with the allowed string', () => {
    ...
    expect(isAllowedAssetUrl('https://notcloudinary.com/a.png', allow)).toBe(
      false,
    );
    expect(isAllowedAssetUrl('https://evilcloudinary.com/a.png', allow)).toBe(
      false,
    );
  });
  ...
  it('rejects an allowed host used as a subdomain of an attacker domain', () => {
    ...
    expect(
      isAllowedAssetUrl('https://res.cloudinary.com.evil.com/a.png', allow),
    ).toBe(false);
    expect(
      isAllowedAssetUrl('https://cloudinary.com.evil.com/a.png', allow),
    ).toBe(false);
    expect(
      isAllowedAssetUrl('https://cdn.mator.uz.evil.com/a.png', allow),
    ).toBe(false);
  });
  ...
  it('rejects an allowed host smuggled into userinfo or path (real host is elsewhere)', () => {
    ...
    expect(
      isAllowedAssetUrl('https://evil.com/res.cloudinary.com/a.png', allow),
    ).toBe(false);
    expect(isAllowedAssetUrl('https://evil.com/?x=cloudinary.com', allow)).toBe(
      false,
    );
  });
  ...
  it('accepts the allowlisted host even with a port or userinfo of its own', () => {
    ...
    expect(
      isAllowedAssetUrl('https://res.cloudinary.com:443/a.png', allow),
    ).toBe(true);
  });
  ...
  it('rejects malformed / non-string values', () => {
    ...
    expect(isAllowedAssetUrl(123, allow)).toBe(false);
  });
});
