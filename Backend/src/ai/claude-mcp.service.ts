...
- price_raw: ЦЕНА РОВНО КАК В ТЕКСТЕ — строкой, со всеми точками/запятыми/пробелами,
  но БЕЗ слова валюты. Пример: текст "130.000 сум" → price_raw: "130.000".
  НЕ преобразуй число сам, НЕ убирай и НЕ добавляй разделители — просто СКОПИРУЙ цифры
  и разделители как есть. Если цены нет — null. (Мы сами приведём строку к числу.)
...
6. НЕ придумывай отсутствующие данные. brand/models бери ТОЛЬКО из ЯВНОГО текста
   (заголовок/описание). НИКОГДА не выводи марку/модель из OEM/GM-номера — по одному
   номеру определить авто нельзя. Если авто в тексте не названо — brand: null, models: [].
   Для расходников (моторное/трансмиссионное масло, тормозная жидкость, антифриз,
   присадки, смазки) НЕ приписывай конкретное авто, если оно не указано в тексте.
...
9. price_raw — цена строкой, СКОПИРОВАННАЯ дословно из текста (без слова валюты). НЕ считай
   сам! Мы применим единый парсер: "130.000"→130000, "1.250.000"→1250000, "130 000"→130000,
   "130,000"→130000, "130.00"→130, "130.50"→130.5. Валюта может быть словами: sum, som, сум,
   сом, so'm, сўм, сoʻм, soʻm, UZS — их в price_raw НЕ включай.
...
Выход: {"title":"Фильтр масла Cobalt Gentra оригинал 96535062 25000 сум","description":"Оригинал","brand":"Chevrolet","models":["Cobalt","Gentra"],"gm_number":"96535062","price_raw":"25000"}

Вход: "Магнитола для Nexia 3\n\nПроизводство Корея, новая"
Выход: {"title":"Магнитола для Nexia 3","description":"Производство Корея, новая","brand":"Chevrolet","models":["Nexia 3"],"gm_number":null,"price_raw":null}

Вход: "Фара передняя\n\nChevrolet Cobalt оригинал 96549774112 130.000 so'm"
Выход: {"title":"Фара передняя","description":"оригинал","brand":"Chevrolet","models":["Cobalt"],"gm_number":"96549774112","price_raw":"130.000"}

Вход (ИСКЛЮЧЕНИЕ — 4 строки заголовок/описание/GM(11)/цена, GM и цена берутся из строк 3 и 4):
"Фара передняя Cobalt\nОригинал, Корея\n96549774112\n350000 сум"
Выход: {"title":"Фара передняя Cobalt","description":"Оригинал, Корея","brand":"Chevrolet","models":["Cobalt"],"gm_number":"96549774112","price_raw":"350000"}

Вход: "Тормозной диск Chevrolet Nexia 3, новый, 97168181, 100000 uzs"
Выход: {"title":"Тормозной диск Chevrolet Nexia 3, новый, 97168181, 100000 uzs","description":"Новый","brand":"Chevrolet","models":["Nexia 3"],"gm_number":"97168181","price_raw":"100000"}

Вход: "генератор 150000"
Выход: {"title":"генератор 150000","description":null,"brand":null,"models":[],"gm_number":null,"price_raw":"150000"}

Вход: "HShshdh (HShha)"
Выход: {"title":null,"description":null,"brand":null,"models":[],"gm_number":null,"price_raw":null}`;
...
export function validateMetadataShape(raw: unknown): ParsedPartMetadata {
  ...
  // Price: the model now returns price_raw (the price string AS WRITTEN), which
  // we run through the SAME shared parsePrice used by every other path — so
  // "130.000" → 130000 regardless of source. We stay backward-compatible with a
  // numeric `price` field in case the model still emits the old shape.
  const price = resolveAiPrice(obj);
...
/**
 * Resolve the price from an AI response. Prefers `price_raw` (a string copied
 * verbatim from the seller text) run through the shared parsePrice, so the
 * improved thousands/decimal rules govern the AI path too. Falls back to a
 * numeric `price` field for backward compatibility. A numeric price is also
 * re-normalized through parsePrice via its string form so an AI-collapsed
 * "130.000"→130 cannot slip through unchanged when sent as a string.
 */
function resolveAiPrice(obj: Record<string, unknown>): number | null {
  const rawStr = obj.price_raw;
  if (typeof rawStr === 'string') return parsePrice(rawStr);
  if (rawStr === null) return null;

  // Backward-compat: a numeric `price`. Trust it as already-numeric (the model
  // was told not to send this shape, but we accept it rather than throwing).
  const priceNum = obj.price;
  if (priceNum === null || priceNum === undefined) return null;
  if (typeof priceNum === 'number') return Number.isFinite(priceNum) ? priceNum : null;
  if (typeof priceNum === 'string') return parsePrice(priceNum);
  throw new Error('ClaudeMcpService: "price"/"price_raw" must be a string, number, or null');
}
