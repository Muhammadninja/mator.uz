  ...
  private registerHandlers() {
    this.bot.start(async (ctx) => {
      ...
      if (seller.status === SellerStatus.ACTIVE) {
        await ctx.reply('✅ Добро пожаловать! Ваш аккаунт активен. Отправьте фото детали с подписью.');
        return;
      }
    ...
    this.bot.on(message('photo'), async (ctx: Context) => {
      ...
      // ── Gate: seller must exist and be ACTIVE ──────────────────────────
      ...
      try {
        ...
        const fileLink = await this.bot.telegram.getFileLink(bestPhoto.file_id);
